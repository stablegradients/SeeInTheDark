...
def encoder_level_2b(inputs,encoder_number,kernelsize=3):
	...
	of encoder
	"""
	conv_1b=slim.conv2d(inputs, encoder_dict[encoder_number]["n"], kernelsize, stride=1, rate=1,padding='SAME',activation_fn=lrelu)
	conv_1b=tf.layers.batch_normalization(conv_1b)
	conv_2b=slim.conv2d(conv_1b, encoder_dict[encoder_number]["n"], kernelsize, stride=1, rate=1,padding='SAME',activation_fn=lrelu)
...
def encoder_level_2a(inputs,encoder_number,kernelsize=3):
	...
	"""
	conv_1a=slim.conv2d(inputs, encoder_dict[encoder_number]["n"], kernel_size=kernelsize, stride=2, rate=1,padding='SAME',activation_fn=lrelu)
	conv_1a=tf.layers.batch_normalization(conv_1a)
	conv_2a=slim.conv2d(conv_1a, encoder_dict[encoder_number]["n"], kernel_size=kernelsize,stride=1, rate=1,padding='SAME',activation_fn=lrelu)
...
def encoder_level_1(inputs,encoder_number):
	...
	input_downsampled=slim.conv2d(inputs, encoder_dict[encoder_number]["n"], kernel_size=3, stride=2, rate=1,padding='SAME',activation_fn=lrelu)
...
def decoder_block(inputs,decoder_number,kernelsize=3):
	...
	dconv_1=slim.conv2d(inputs, decoder_dict[decoder_number]["m"]//4, kernel_size=1, stride=1, rate=1,padding='SAME',activation_fn=lrelu)
	...
	dconv_3=slim.conv2d(dconv_2,decoder_dict[decoder_number]["m"]//4,kernel_size=1,stride=1,rate=1,padding='SAME',activation_fn=lrelu)
...
def pre_encoder(inputs):
	...
	"""
	pre_enc_1=slim.conv2d(inputs,encoder_dict["1"]["m"], kernel_size=7, stride=2, rate=1,padding='SAME',activation_fn=lrelu)
	pre_enc_2=slim.conv2d(pre_enc_1,encoder_dict["1"]["m"], kernel_size=3, stride=2, rate=1,padding='SAME',activation_fn=lrelu)
	return pre_enc_2
def post_decoder(inputs,classes):
	...
	post_decoder_3=slim.conv2d_transpose(post_decoder_2,post_decoder_dict["c"]["n"],kernel_size=3,stride=2,padding='SAME',activation_fn=tf.nn.tanh)
