...
def network(inputs,reuse=None, is_training=True):
	with tf.variable_scope('Generator',reuse=reuse):
		...
		with slim.arg_scope([slim.conv2d],padding='SAME',weights_regularizer=slim.l2_regularizer(0.001)):
			...
			conv1 = slim.conv2d(inputs, 16, [7, 7], rate=1, activation_fn=lrelu,reuse=reuse, stride=1)
			conv1 = slim.conv2d(conv1, 16, [7, 7], rate=1, activation_fn=lrelu,reuse =reuse, stride=2)
			conv1 =  slim.batch_norm(conv1, is_training=is_training)
			
			conv2 = slim.conv2d(conv1, 32, [5, 5], rate=1, activation_fn=lrelu, reuse=reuse, stride=1)
			...
			conv9 = slim.conv2d(up3, 16,[5, 5], rate=1, activation_fn=lrelu, reuse=reuse, stride=1)
			conv9 = slim.conv2d(conv9, 16, [5, 5], rate=1, activation_fn=lrelu, reuse=reuse, stride=1)
			...
			conv10 = slim.conv2d(up2, 12, [7, 7], rate=1, activation_fn=lrelu, reuse=reuse, stride=1)
			conv10 = slim.conv2d(conv10, 12, [7, 7], rate=1, activation_fn=lrelu, reuse=reuse, stride=1)
			up1 = tf.depth_to_space(conv10,2)

			conv11 = slim.conv2d(up1, 3, [7, 7], rate=1, activation_fn=lrelu, reuse=reuse, stride=1)
			conv11 = slim.conv2d(conv11, 3, [7, 7], rate=1, activation_fn=tf.nn.tanh, reuse=reuse, stride=1)
